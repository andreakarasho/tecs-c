/*
 * TinyEcs Core Performance Benchmark (Optimized)
 *
 * Optimizations applied:
 * - Compile with -O3 -march=native -flto
 * - Reuse query iterator across frames
 * - Use restrict pointers for better vectorization
 * - Remove unnecessary tecs_world_update() calls
...
/* Hot-path update loop with restrict pointers for vectorization */
static inline void update_entities(Position* restrict positions,
                                   const Velocity* restrict velocities,
                                   int count) {
    /* Compiler can now vectorize this with SIMD */
...
int main(void) {
    printf("=== TinyEcs Core Performance Benchmark (OPTIMIZED) ===\n");
    ...
    for (int i = 0; i < ENTITIES_COUNT; i++) {
        ...
        Position pos = {1.0f, 1.0f};  /* Non-zero for more realistic computation */
        Velocity vel = {1.0001f, 1.0001f};  /* Slight multiplier */
    ...
    while (true) {
        ...
        for (int frame = 0; frame < FRAMES_PER_BATCH; frame++) {
            ...
            while (tecs_query_next(iter)) {
                ...
                /* Call optimized update function */
                update_entities(positions, velocities, count);
            }
            tecs_query_iter_free(iter);

            /* NOTE: Removed tecs_world_update() - not needed for this benchmark */
        }
